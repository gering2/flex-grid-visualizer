...
import { GRID_COLS, GRID_ROWS, PLACE_ITEMS, GAP_VALUES, JUSTIFY_ITEMS, ALIGN_ITEMS, JUSTIFY_CONTENT_GRID, ALIGN_CONTENT_GRID } from '../../constants';
      ...
      <ControlGroup label="Justify Items" propertyKey="justify-items">
        {JUSTIFY_ITEMS.map(opt => (
          <ControlButton
            ...
            onClick={() => setGrid(g => ({ ...g, justifyItems: opt.value }))}
            label={opt.label}
          />
        ))}
      </ControlGroup>
      <ControlGroup label="Align Items" propertyKey="align-items">
        {ALIGN_ITEMS.map(opt => (
          <ControlButton
            ...
            onClick={() => setGrid(g => ({ ...g, alignItems: opt.value }))}
            label={opt.label}
          />
        ))}
      </ControlGroup>
